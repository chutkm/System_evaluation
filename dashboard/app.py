...
# from sqlalchemy import create_engine
# from bot.config import DATABASE_SYNC_URL
# from showcase_agent.llm_recommendations import get_teacher_recommendation
...
# teacher_filter = st.sidebar.selectbox(
#     "Преподаватель",
#     ["Все"] + sorted(df["teacher_name"].dropna().unique())
# )
...
# if teacher_filter != "Все":
#     df = df[df["teacher_name"] == teacher_filter]
...
# # )
# teacher_sentiments = (
#     df.groupby("teacher_name")["sentiment"]
#       .value_counts(normalize=True)
#       .unstack()
#       .fillna(0)
# )
...
#     if col not in teacher_sentiments.columns:
#         teacher_sentiments[col] = 0

# negative_teachers = teacher_sentiments.sort_values(
#     by="negative",
#     ascending=False
# ).head(5)
...
# # Загружаем feedback с join по таблице teachers
# df = pd.read_sql("""
# SELECT 
#     f.*,
#     t.id AS teacher_id,
...
# # ----------------- Преобразуем topics из JSON -----------------
# df["topics"] = df["topics"].apply(lambda x: json.loads(x) if isinstance(x, str) else x)
...
# col1.metric("Средняя оценка", avg_rating)
# col2.metric("Количество отзывов", len(df))
# col3.metric("Негативные отзывы", (df["sentiment"] == "negative").sum())
...
# st.subheader("📊 Распределение оценок")
# fig_rating = px.histogram(df, x="rating", nbins=5)
# fig_rating.update_xaxes(tickmode="linear", dtick=1)
# st.plotly_chart(fig_rating, use_container_width=True)
...
# teacher_sentiments = (
#     df.groupby(["teacher_id", "teacher_name"])["sentiment"]
#       .value_counts(normalize=True)
#       .unstack()
#       .fillna(0)
# )
